...
def _usage_guide_text() -> str:
    return (
        ...
        "  Auction Market Theory (AMT) indicators — session-scoped profiles:\n"
        "    VWAP Bands: vwap_session, vwap_upper_1/2, vwap_lower_1/2\n"
        "    Volume Profile: vp_poc, vp_vah, vp_val (per session)\n"
        "    Rolling VP: vp_poc_Nd, vp_vah_Nd, vp_val_Nd (any window N)\n"
        "    Market Profile: mp_poc, mp_vah, mp_val (TPO-based)\n"
        "    Auction State: inside_value, above_value, below_value,"
        " at_poc, near_vah, near_val, distance_to_vah_pct, "
        "distance_to_val_pct, value_area_width_pct, balance_status\n"
        "    AMT Signals: acceptance_into_value, rejection_from_edge,"
        " acceptance_outside_value, poc_rejection, edge_volume_building,"
        " value_area_migration\n"
        "    Best on intraday data (5min/30min/1h). On daily bars, use"
        " rolling composites (vp_poc_Nd) for multi-day value areas.\n\n"
        ...
        "AMT fixed: vwap_session, vwap_upper_1/2, vwap_lower_1/2, "
        "vp_poc, vp_vah, vp_val, vp_poc_5d/20d, vp_vah_5d/20d, "
        "vp_val_5d/20d (rolling composites via vp_poc_Nd for any N), "
        "mp_poc, mp_vah, mp_val, inside_value, above_value, below_value, "
        "at_poc, near_vah, near_val, balance_status, "
        "acceptance_into_value, acceptance_outside_value, "
        ...
        "risk flags\n"
        "  AMT: Auction Market Theory indicators — VWAP SD bands, "
        "Volume Profile (POC/VAH/VAL), Market Profile (TPO), "
        "auction state classifiers, AMT rule signals. "
        "Use compute_indicators or compute_trading_metrics.\n"
