...
def _usage_guide_text() -> str:
    return (
        ...
        # ── Enrichment modes ──
        "ENRICHMENT MODES (IMPORTANT — read before backtesting)\n"
        "  Finbar supports two enrichment modes for computing indicators:\n\n"
        "  live_parity_streaming (DEFAULT, RECOMMENDED):\n"
        "    Each enriched row is built using ONLY bars available at that "
        "bar's close — no future lookahead. This faithfully reproduces "
        "what would occur in live trading. Required for strategies using "
        "session-scoped indicators: vp_poc, vp_vah, vp_val, near_val, "
        "above_value, auction state, profile_shape, wyckoff_phase, "
        "and their derivatives. All backtest defaults use this mode.\n\n"
        "  batch_full_frame (RESEARCH ONLY):\n"
        "    Legacy approach that computes all indicators on the entire "
        "frame at once. Session VP/MP indicators broadcast completed-session "
        "values to earlier rows — LOOKAHEAD BIAS. Only use for:\n"
        "    • TA-only strategies (sma_N, ema_N, rsi_N, macd, bbands, atr)\n"
        "    • Comparing against old pre-causal research results\n"
        "    • Deliberate what-if experiments\n"
        "    Batch artifacts (compute_indators, compute_trading_metrics) "
        "are produced as batch_full_frame. Causal backtests use the "
        "streaming enricher built into the strategy runtime package — "
        "they do NOT re-use batch artifacts.\n\n"
        "  backtest_strategy_definition accepts enrichment_mode "
        "(default: live_parity_streaming). run_strategy_pipeline also "
        "exposes it. Results include live_parity_safe and parity_warnings "
        "so you can verify the mode was applied correctly.\n\n"
        ...
        "cancel_job(job_id)\n\n"
        # ── Indicators ──
        "INDICATORS & METRICS (200+ across 30 families)\n"
        "  apply_indicators(bars_json, indicators_json) → indicator bars. "
        "Use for small datasets (<1000 bars). Full catalog supports:\n"
        "    Traditional TA: sma_N, ema_N, rsi_N, macd, atr, adx, "
        "vwap, bb_upper/middle/lower, ibs, rvol, ker, kama.\n"
        "    Trend/Breakout: trend_direction/strength/status, "
        "swing_high/low_20, breakout_level/signal/quality, etc.\n"
        "    AMT: vwap_session & SD bands, vp_poc/vah/val (session), "
        "vp_poc_Nd (any N), rvp_poc_N, cvp_poc_Nd, mp_poc/vah/val, "
        "auction state classifiers & rule signals.\n"
        "    Profile/Wyckoff: profile_shape, is_*_shape, is_coiled, "
        "coil_intensity, wyckoff_phase, poc_slope_5/20, is_* phase bools.\n"
        "    Market Microstructure: Spread (7), Volatility (9), "
        "Liquidity/impact (6), Order flow (6), Informed trading (2), "
        "Jump risk (4), Resiliency (3), Seasonality (4).\n"
        "    Intraday Realized: realized_vol_5m/15m/1h, "
        "bipower_variation, realized_skewness/kurtosis, lee_mykland_jump.\n"
        "    Price Action: Fibonacci (5), Bill Williams (9), "
        "SMC/FVG/order blocks (11), VSA (10), Supply/demand zones (8), "
        "Trend structure (6), Hurst/regime (4).\n"
        "    Proxies: proxy_vwap, proxy_atr, proxy_ibs, proxy_parkinson, "
        "proxy_garman_klass, proxy_rogers_satchell, proxy_expected_move, etc.\n"
        "    Derivatives (crypto): funding_rate, OI, CVD, "
        "long_short_ratio, liquidations (CoinGlass).\n"
        "  AMT indicators work best on intraday data (5min/30min/1h). "
        "On daily bars, use rolling composites (vp_poc_Nd).\n"
        "  Discover: list_market_metrics, check_metric, "
        "resolve_metric, get_strategy_capabilities.\n\n"
        ...
        "200+ indicators across 30 families (see list_market_metrics). "
        "New families: Market Microstructure, Price Action, "
        "Profile/Wyckoff classifiers, Intraday Realized, "
        ...
        "  Catalog: list_market_metrics, check_metric, "
        "resolve_metric — discover 200+ indicators across 30 families\n"
        "  Signals: compute_signals — confidence scores, RSI zones, "
        "risk flags\n"
