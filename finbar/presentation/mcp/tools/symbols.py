...
def _usage_guide_text() -> str:
    return (
        ...
        "DATA SOURCES\n"
        "  • yfinance — Yahoo Finance (stocks, ETFs). Rate-limited (~30 req/min).\n"
        "  • hyperliquid — Hyperliquid DEX (spot, perp, HIP-3). Rate-limited "
        "(1200 weight/min, token bucket).\n\n"
        "HYPERLIQUID MARKET TYPES\n"
        "  • spot — Plain tickers like BTC, PURR, ETH\n"
        "  • perp — Perpetual futures like BTC, ETH, SOL\n"
        "  • hip3 — Third-party DEX perps with dex:COIN format (e.g., flx:TSLA)\n"
        "  Use list_hyperliquid_tickers('perp') to discover available tickers.\n\n"
        "INTERVALS\n"
        "  5min, 30min, 1h, 1d, 1w\n"
        "  • Intraday (5min, 30min): Yahoo caps at ~60 days. Hyperliquid: ~17 days.\n"
        "  • 1h: Yahoo caps at ~730 days. Hyperliquid: ~208 days.\n"
        "  • Daily/Weekly: Full history available (Hyperliquid: ~3 years).\n\n"
        "TWO DATA PATHS — CRITICAL CONCEPT\n"
        "  • FRESH: fetch_price_history(symbol, interval, source='yfinance') — "
        "fetches from source, saves to cache, returns job_id. Rate-limited and "
        "async — poll with get_job_progress() and retrieve with "
        "get_job_results().\n"
        "  • CACHED: get_cached_prices(symbol, interval, source='yfinance') — "
        "reads from local SQLite. Instant, no rate limits. Only returns "
        "previously-fetched data.\n\n"
        "TYPICAL WORKFLOW (STOCKS)\n"
        ...
        "TYPICAL WORKFLOW (CRYPTO)\n"
        "  1. list_hyperliquid_tickers('perp') → discover tickers\n"
        "  2. fetch_price_history('BTC', '1d', source='hyperliquid') → job_id\n"
        "  3. get_cached_prices('BTC', '1d', source='hyperliquid') — instant\n\n"
        "ALL TOOLS\n"
        "  • get_symbol_info(symbol) — company/asset metadata\n"
        "  • get_latest_quote(symbol, source='yfinance') — most recent bar\n"
        ...
        "    → job_id (background async)\n"
        "  • get_job_progress(job_id) — poll status + progress %\n"
        "  • get_job_results(job_id) — retrieve completed results\n"
        ...
        "  • list_hyperliquid_tickers(market_type) — discover HL tickers\n"
    )
