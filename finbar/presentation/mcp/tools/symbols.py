...
def _usage_guide_text() -> str:
    return (
        ...
        "  start_enrichment_job(symbol, source, interval, mode, indicators_json, "
        "definition_json) → async server-side enrichment from cached bars\n"
        "  get_enrichment_job_progress(job_id) → poll enrichment progress\n"
        "  get_enrichment_job_results(job_id, page=0, page_size=500) → paginated "
        "enriched bars\n"
        ...
        "bars, or use start_enrichment_job for large cached datasets\n"
        ...
        "    start_enrichment_job(...) → async indicators/features from cached "
        "bars, no huge bars_json input\n"
        "    get_enrichment_job_progress(job_id) → status/stage/progress\n"
        "    get_enrichment_job_results(job_id, page, page_size) → paged "
        "enriched bars\n"
        "    backtest_strategy_json(..., bars_artifact_id=job_id) → backtest "
        "without round-tripping enriched bars\n"
        ...
        "  4. start_enrichment_job('AAPL', mode='strategy_required', "
        "definition_json=definition) → enrichment_job_id\n"
        "  5. backtest_strategy_json(definition, bars_artifact_id="
        "enrichment_job_id) → no large JSON round trip\n\n"
