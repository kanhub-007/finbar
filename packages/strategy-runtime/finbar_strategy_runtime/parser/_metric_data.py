"""Static registry of market metric definitions (data only).

Split from ``_metric_registry.py`` to keep the flat METRICS list below the
500-line review threshold. Both files together replace the single original.

Importers should continue to import from ``_metric_registry``, which
re-exports ``METRICS`` for backward compatibility.
"""
