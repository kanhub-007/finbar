...
all accounting is done through SimulationState.cash directly.
"""
...
class MarginAccountManager:
    ...
    Delegates to SimulationState for trade recording and position storage;
    ...
    def lock_entry_margin(
        self, state: SimulationState, cost: float, commission: float
    ) -> None:
    ...
    def sync_state_equity(self, state: SimulationState) -> None:
        """Sync SimulationState.cash to margin account equity."""
        state.cash = self.account.equity

    def apply_funding(self, position: SimulatedPosition) -> float:
    ...
    def check_margin_call(self, position: SimulatedPosition, close: float) -> str | None:
