...
_ALLIGATOR_CACHE_KEY = "__alligator_lines"


def _alligator_lines_cached(df, cache):
    """Return (jaw, teeth, lips) for the frame, computing once per call.

    The four alligator handlers (jaw/teeth/lips/status) all derive from the
    same three rolling SMAs. Without this cache each handler recomputed the
    full alligator, so requesting all four columns did 4x the work. The
    cache dict is the per-calculate() cache shared across handlers.
    """
    cached = cache.get(_ALLIGATOR_CACHE_KEY)
    if cached is not None:
        return cached
    lines = _alig_lines(df["high"], df["low"])
    cache[_ALLIGATOR_CACHE_KEY] = lines
    return lines
...
def _h_alig_jaw(df, _name, cache):
    jaw, _teeth, _lips = _alligator_lines_cached(df, cache)
    df["alligator_jaw"] = jaw
    return df


@_register("alligator_teeth", requires={"high", "low"})
def _h_alig_teeth(df, _name, cache):
    _jaw, teeth, _lips = _alligator_lines_cached(df, cache)
    df["alligator_teeth"] = teeth
    return df


@_register("alligator_lips", requires={"high", "low"})
def _h_alig_lips(df, _name, cache):
    _jaw, _teeth, lips = _alligator_lines_cached(df, cache)
    df["alligator_lips"] = lips
    return df


@_register("alligator_status", requires={"high", "low"})
def _h_alig_status(df, _name, cache):
    jaw, teeth, lips = _alligator_lines_cached(df, cache)
...
def _h_er_div(df, _name, _cache):
    df["effort_result_divergence"] = _er_div(
